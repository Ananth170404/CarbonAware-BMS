...
# --- Advanced Professional CSS for Dark Theme ---
st.markdown("""
    <style>
        ...
        .main-title {
            ...
            font-weight: 900;
            text-align: center;
            background: linear-gradient(90deg, #00d4ff 0%, #7a5cff 50%, #ff00ff 100%);
        ...
        .stTabs [aria-selected="true"] {
            background: linear-gradient(90deg, #00d4ff 0%, #7a5cff 100%);
        ...
        hr {
            border: none;
            height: 2px;
            background: linear-gradient(90deg, transparent, #7a5cff, transparent);
            margin: 2em 0;
        }
...
st.sidebar.markdown("""
    <div style='text-align:center; padding:20px 0;'>
        <h1 style='font-family:Orbitron; color:#00d4ff; font-size:1.8em; letter-spacing:2px;'>ELECTRICITY</h1>
        <p style='font-family:Poppins; color:#7a5cff; font-size:1em;'>Analytics Platform</p>
    </div>
""", unsafe_allow_html=True)

st.sidebar.markdown("---")
st.sidebar.markdown("<h3 style='color:#00d4ff; font-family:Poppins;'>Filters & Controls</h3>", unsafe_allow_html=True)
...
st.sidebar.markdown("---")
chart_theme = st.sidebar.radio("Chart Theme", options=["plotly_dark", "plotly"], index=0)
...
with tab2:
    ...
    if show_history and not hist_f.empty:
        for node_id in hist_f["node_id"].unique():
    ...
    if show_predictions and not pred_f.empty:
        ...
        for node_id in pred_f["node_id"].unique():
    ...
    if show_anomalies and not anom_f.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            name="Anomaly",
            marker=dict(size=10, symbol='x', color='#ff00ff', line=dict(width=2))
        ))
...
with tab3:
    ...
    if not hist_f.empty:
        ...
        fig_daily.add_trace(
            go.Bar(x=agg["date"], y=agg["sum"], name="Sum", marker_color='#00d4ff'),
            row=1, col=1
        )
        fig_daily.add_trace(
            go.Scatter(x=agg["date"], y=agg["mean"], name="Average", mode='lines+markers', marker_color='#7a5cff'),
            row=1, col=2
        )
        fig_daily.add_trace(
            go.Scatter(x=agg["date"], y=agg["max"], name="Maximum", mode='lines+markers', marker_color='#ff00ff'),
            row=2, col=1
        )
        fig_daily.add_trace(
            go.Scatter(x=agg["date"], y=agg["min"], name="Minimum", mode='lines+markers', marker_color='#00ff88'),
            row=2, col=2
        )
...
st.markdown("---")
st.markdown("""
    <div style='text-align:center; padding:20px; font-family:Roboto; color:#7a5cff;'>
        <p>Electricity Analytics Platform | Data Pipeline: /opt/electricity-pipeline/output_local</p>
        <p style='font-size:0.9em; color:#b8bcc4;'>Professional Dashboard | Real-time Monitoring & Forecasting</p>
    </div>
""", unsafe_allow_html=True)
